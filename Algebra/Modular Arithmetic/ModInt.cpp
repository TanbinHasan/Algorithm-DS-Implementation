...
class Modular {
 public:
  ...
  Modular() : v(0) {}
  Modular(__int128_t _v) : v(int(_v % MOD)) { v += (v < 0) * MOD; }
  ...
  friend Modular power(Modular b, __int128_t p) {
    Modular res = 1;
    while (p > 0) {
      if (p & 1) res *= b;
      b *= b;
  ...
  Modular& operator/=(Modular o) {
    v = int(1LL * v * inv(o) % MOD);
    return *this;
  }
  friend Modular operator+(Modular a, Modular b) { return a += b; }
  friend Modular operator-(Modular a, Modular b) { return a -= b; }
  friend Modular operator*(Modular a, Modular b) { return a *= b; }
  friend Modular operator/(Modular a, Modular b) { return a *= inv(b); }
};
