...
class SegmentTree {
 ...
 public:
  ...
  vector<int> intersecting_segments(vector<int>& a) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      update(left[a[i]], 1);
      ans[a[i]] = (i - left[a[i]] + 1) - 2 * query(left[a[i]], i);
      // total length - 2 * number of nested segments
    }
    return ans;
  }
};
...
int main(void) {
  ...
  auto ans = st.intersecting_segments(a);
